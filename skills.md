...
- **codegraph** (`colbymchenry/codegraph`, MCP + CLI) — structural
  code-knowledge-graph: call flow, callers/callees, blast radius. Installed
  globally (`npm i -g @colbymchenry/codegraph`) and wired into Claude Code
  (`codegraph install --target claude --location global`); indexed per repo
  with `codegraph init` (auto-syncs on file changes, `.codegraph/` is
  git-ignored). Currently has no source files to index here (docs/HTML-only
  repo) — becomes useful once real app code lands.

**Everyone on this project should have these installed locally** — see
[New collaborator setup](#new-collaborator-setup) below.

## New collaborator setup

`superpowers`, `code-review`, and `pr-review-toolkit` install as plugins
(`/plugin`). The three below don't — `mcp-builder` and `webapp-testing` are
plain skill folders, and `codegraph` is a separate CLI/MCP. Run these once
per machine:

```bash
# mcp-builder + webapp-testing (from anthropics/skills, user scope)
git clone --depth 1 https://github.com/anthropics/skills.git /tmp/anthropics-skills-dl
cp -r /tmp/anthropics-skills-dl/skills/mcp-builder ~/.claude/skills/mcp-builder
cp -r /tmp/anthropics-skills-dl/skills/webapp-testing ~/.claude/skills/webapp-testing
rm -rf /tmp/anthropics-skills-dl

# codegraph (CLI + MCP wiring, then per-repo index)
npm i -g @colbymchenry/codegraph
codegraph install --target claude --location global --yes
cd skillbridge && codegraph init
```

Restart Claude Code after the `codegraph install` step so it picks up the
new MCP server.
...
- **2026-09-20 — mcp-builder + webapp-testing** (from `anthropics/skills`,
  installed at `~/.claude/skills/`) — pulled in on request. obra/superpowers
  was already the source of the Active `superpowers` plugin (no action
  needed); of anthropics/skills' 12 not-yet-active skills, picked
  mcp-builder and webapp-testing specifically rather than the full
  ...
  marketplace. Promoted to Active; setup steps added to "New collaborator
  setup" since this is a per-machine, not per-repo, install.
- **2026-09-20 — codegraph** (`colbymchenry/codegraph`) — promoted from seed
  catalog to Active on request; installed CLI + MCP wiring + local index (see
  "New collaborator setup" for the exact commands).
